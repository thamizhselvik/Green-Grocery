...
// --- CSS (Ideally, put this in a separate FarmerSignup.css file) ---
const styles = `
  .btn-signup {
...
const FarmerSignup = () => {
    ...
    const validate = () => {
        ...
        const uzhavanRegex = /^[a-zA-Z0-9]+$/; // Basic alphanumeric check, adjust if needed

        if (!formData.name.trim()) newErrors.name = "Name is required";
        if (!formData.username.trim()) newErrors.username = "Username is required";
        if (!formData.password) newErrors.password = "Password is required"; // No trim for password
        ...
        // Example: Add stricter validation if Uzhavan ID has a specific format
        if (!formData.uzhavan.trim()) newErrors.uzhavan = "Uzhavan ID is required";
        // else if (!uzhavanRegex.test(formData.uzhavan)) newErrors.uzhavan = "Invalid Uzhavan ID format";
    ...
    const checkExistence = async (field, value, endpoint) => {
        ...
        try {
            const response = await axios.post(`http://localhost:3000/api/farmer/farmer/${endpoint}`, {
                [field]: value,
            });
    ...
    const verify = async (e) => {
        ...
        try {
            ...
            const results = await Promise.all([
                ...
                checkExistence("uzhavan", formData.uzhavan, "checkuzhavar"), // Changed endpoint name slightly
            ]);

            const [usernameExists, aadharExists, contactExists, uzhavanExists] = results;
            ...
            if (uzhavanExists) newErrors.uzhavan = "Uzhavan ID already registered";
            ...
            await axios.post("http://localhost:3000/api/farmer/add", formData);
            ...
            toast.success("Registration Successful! Redirecting to login...", {
                ...
                theme: "light",
                transition: Bounce,
                onClose: () => navigate("/farmerlogin") // Navigate after toast closes
            });

            navigate("/FarmerDashboard")

        } catch (err) {
            console.error("Error during signup:", err.response?.data || err.message || err);
            const errorMsg = err.response?.data?.message || "Registration failed. Please try again.";
            toast.error(errorMsg, { transition: Bounce });
            // Optionally set errors based on server response
        ...
        <> {/* Use Fragment to inject styles */}
            ...
            <div className="container mt-5 mb-5"> {/* Added mb-5 for bottom spacing */}
                <div className="row justify-content-center">
                    <div className="col-md-10 col-lg-8"> {/* Slightly wider on medium screens */}
                        <div className="card shadow p-4 p-md-5 card-signup"> {/* Increased padding on medium screens */}
                            <h3 className="text-center mb-4">Farmer Registration</h3>
                            <form onSubmit={verify} noValidate> {/* Added noValidate to disable default browser validation */}
                                <div className="row">
                                    {/* Name */}
                                ...
                                <div className="row">
                                    ...
                                    <div className="col-md-6 mb-3">
                                        ...
                                        <input
                                            type="text" // Keep as text to allow leading zeros if any, validation handles format
                                ...
                                <div className="row">
                                    {/* Uzhavan ID */}
                                    <div className="col-md-6 mb-3">
                                        <label htmlFor="uzhavanInput" className="form-label">Uzhavan ID</label>
                                        <input
                                            type="text"
                                            className={`form-control ${errors.uzhavan ? 'is-invalid' : ''}`}
                                            id="uzhavanInput"
                                            name="uzhavan"
                                            value={formData.uzhavan}
                                            onChange={handleChange}
                                            required
                                        />
                                        {errors.uzhavan && <div className="invalid-feedback">{errors.uzhavan}</div>}
                                    </div>
                                ...
                                <div className="mb-3"> {/* Full width */}
                                    <label htmlFor="addressInput" className="form-label">Full Address</label>
                                    <textarea
                                ...
                                <div className="row">
                                    ...
                                    <div className="col-md-6 mb-3">
                                        ...
                                        <select
                                            className={`form-select ${errors.state ? 'is-invalid' : ''}`} // Use form-select for <select>
                                            id="stateSelect"
                                            name="state"
                                        ...
                                        >
                                            ...
                                            {Object.keys(indianStates).sort().map((state) => ( // Sort states alphabetically
                                    ...
                                    <div className="col-md-6 mb-3">
                                        ...
                                        <select
                                            className={`form-select ${errors.city ? 'is-invalid' : ''}`} // Use form-select
                                            id="citySelect"
                                            name="city"
                                            value={formData.city || ""}
